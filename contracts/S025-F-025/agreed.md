# S025-F-025 — Chunking operator: agreed contract

> Derived from `proposed.md` with all 7 findings from `feedback.md` addressed:
> F1 BLOCKER (pyarrow version in R1), F2 HIGH (D10 nullable justification),
> F3 HIGH (verification plan missing extract_mineru step), F4 MEDIUM (unit tests
> in chunks) layer body), F5 MEDIUM (all) insertion after lance), F6 NIT (§3-B
> premise correction), F7 NIT (worker MINIO_* completeness — Option A chosen).
...
Dagster-layer change plus infrastructure plumbing (new Python packages in the Dagster image, new
...
| `docker/docker-compose.dev.yml` | **EDIT** | Add `MINIO_LANCE_BUCKET: ${MINIO_LANCE_BUCKET:-lance}` to the `environment:` block of `dagster-webserver` and `dagster-daemon`. Add the full MINIO_* block (`MINIO_ENDPOINT`, `MINIO_ROOT_USER`, `MINIO_ROOT_PASSWORD`, `MINIO_LANCE_BUCKET`) to `dagster-worker-cpu` and `dagster-worker-heavy` — complete parity with webserver/daemon (F7 fix: Option A). |
| `verify/checks.sh` | **EDIT** | Add a new `chunks)` layer (end-to-end verification of the chunking pipeline). Add `chunks` to the `all)` chain **after `lance`** (not after `extract` — F5 fix). |
...
### 3-B. `docker/docker-compose.dev.yml` — new env vars

The env var `MINIO_LANCE_BUCKET` is read by `chunker.py`'s `build_lance_storage_options()` to
form the S3 URI `s3://{MINIO_LANCE_BUCKET}/chunks`. The `fastapi` service does NOT have this
variable in its `docker-compose.dev.yml` `environment:` block; it resolves to `"lance"` via the
Pydantic Settings Python default `MINIO_LANCE_BUCKET: str = "lance"` in `config.py`. Dagster's
`chunker.py` uses `os.environ` (no default), so explicit injection into docker-compose is required
for all four Dagster services.

**`dagster-webserver` and `dagster-daemon`** already have `MINIO_ENDPOINT`, `MINIO_ROOT_USER`,
`MINIO_ROOT_PASSWORD`. Add only `MINIO_LANCE_BUCKET`:

```yaml
MINIO_LANCE_BUCKET: ${MINIO_LANCE_BUCKET:-lance}
```

**`dagster-worker-cpu` and `dagster-worker-heavy`** currently have only `WORKER_PROFILE` and
Postgres vars; they have no MINIO_* vars at all. To avoid an inconsistent state where
`MINIO_LANCE_BUCKET` is present but the other required MINIO vars are absent (which would cause
`build_lance_storage_options()` to fail on the missing vars if a worker is ever activated),
add the full MINIO_* block to both worker services:
...
The default `lance` for `MINIO_LANCE_BUCKET` is safe: in production the `.env` override controls
the value; in dev the default matches the bucket created by the MinIO init container.
...
logic — duplication is safe; any future schema change touches both files together. The copy in
`chunker.py` MUST NOT add `nullable=False` to any field that uses the default `nullable=True` in
the F-023 authoritative copy, because Arrow schema comparison is strict on nullability and a
mismatch would cause lancedb to raise on `create_table(..., exist_ok=True)`.
...
Because there is no automatic graph dependency, the `chunks)` verification layer in `checks.sh`
MUST trigger `extract_mineru` explicitly and wait for it to complete before triggering `chunks`
(see V1 steps below).
...
as a convention choice: while `pa.string()` fields are `nullable=True` by default in pyarrow
(confirmed: `pa.field('docling_refs', pa.string()).nullable == True`) and `None` would be
accepted by the schema, using `""` maintains consistent non-null sentinel semantics for string
ref fields. Real ref values will be populated in F-026 (progressive enrichment per design doc
§4.2 note). `attr_pii_categories` and `attr_minhash_signature` are list types and are set to
`None` (null).
...
6. In the `fastapi` container, run a Python one-liner:
   ```bash
...
### `chunks)` layer structure in checks.sh

The `chunks)` case follows the `extract)` pattern: unit tests run FIRST, then E2E steps.
...
chunks)
  ...
  # F025-setup: upload a test PDF, capture source_id
  ...

  # F025-step2: trigger extract_mineru (prerequisite — no graph dep per D9)
  POST /api/runs?asset=extract_mineru&source_ids=[{source_id}]
  poll until COMPLETED_SUCCESS

  # F025-step3: trigger chunks
  POST /api/runs?asset=chunks&source_ids=[{source_id}]
  poll until SUCCESS

  # V1–V4: Lance query verification (fastapi container)
  ...
```

### `all)` chain

Add `chunks` to the `all)` chain **after `lance`**:
...
This ordering reflects the dependency chain: the Lance table (`lance)`) must be initialised before
`chunks)` writes to it, and `chunks)` is self-contained (it triggers its own `extract_mineru` step
internally, so position relative to `extract)` is irrelevant for correctness but position after
`lance)` makes the dependency chain legible).

### Unit tests (local, inside dagster-webserver container)

The unit test invocation is the **first step** of the `chunks)` case in `checks.sh` (see layer
structure above). Tests can also be run directly:
...
| **#3 Schema frozen post-publish** | No in-place schema edits after publish | `CHUNKS_SCHEMA` in `chunker.py` is a copy of the agreed schema from F-023 (`lance.py`). No `nullable=False` added to any field. No schema change to the existing table. If schema must change in future, a new version is required. |
...
lancedb, the Dockerfile must be updated in the same commit. *Mitigation*: pin both to
`lancedb==0.30.2` and `pyarrow==24.0.0` respectively; CI will catch any future drift via
