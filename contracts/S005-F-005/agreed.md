...
## Addenda (resolved during implementation — 2026-05-22)

### Addendum 1 — OQ-1: Confirmed selector names for `launchRun` mutation

Introspected against running Dagster 1.11.16 instance at `http://localhost:13000/graphql`:

```
Query: { repositoriesOrError { ... on RepositoryConnection { nodes { name location { name } jobs { name } } } } }
```

Result:
- `repositoryLocationName` = `"dagster_platform.definitions"`
- `repositoryName` = `"__repository__"`
- `jobName` = `"hello_world_job"` (confirmed visible after rebuild + restart)

These values match the draft variables in §4.1. No change to the mutation variables.

The dagster-webserver image must be rebuilt (not just restarted) when `dagster_platform/definitions.py` changes, because it has no bind-mount — the code is baked into the image via `COPY dagster/ /app/dagster/` in `docker/dagster/Dockerfile`. A `docker compose build --no-cache dagster-webserver dagster-daemon && docker compose up -d dagster-webserver dagster-daemon` is required after modifying definitions.py.

### Addendum 2 — OQ-2: Confirmed run query field name and not-found type

Introspected against running Dagster 1.11.16 instance:

**Both** `pipelineRunOrError` **and** `runOrError` exist in the schema. Both are valid.

Testing with a non-existent run ID confirms:
- `pipelineRunOrError(runId: "nonexistent")` → `{"__typename": "RunNotFoundError", ...}`
- `runOrError(runId: "nonexistent")` → `{"__typename": "RunNotFoundError", ...}`

**Decision:** Use `pipelineRunOrError` (matches the candidate query in §4.2) because it is the legacy name that has existed across all Dagster 1.x versions and is confirmed present in 1.11.16. The modern alias `runOrError` also works but `pipelineRunOrError` is safer for cross-version compatibility.

**Confirmed not-found type:** `RunNotFoundError` (NOT `PipelineRunNotFoundError` — that type exists in the schema but is NOT returned by `pipelineRunOrError` for a missing run). The `... on PipelineRunNotFoundError` spread in the candidate query §4.2 must be replaced with `... on RunNotFoundError`.

**Confirmed fragment types for `pipelineRunOrError`:**
- `Run` — success, extract `.status`
- `RunNotFoundError` — run does not exist, raise `DagsterRunNotFoundError`
- `PythonError` — gateway error, raise `DagsterGatewayError`

### Addendum 3 — OQ-3: Executor behaviour observed

The `hello_world_job` runs via the `multiprocess_executor` (Dagster default when launched via `launchRun`). The daemon picks the run from the queue and dispatches to a subprocess. Observed completion time: under 5 seconds. The 60-second poll window is adequate.
