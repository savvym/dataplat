# Agreed — S038-F-038: List Recipes (GET /api/recipes)

> Mode A reviewer verdict: **APPROVED** with three minor findings folded in below
> (F1 MEDIUM pagination — Option A confirmed; F2 LOW SQL-structure inspection test
> added to §6; F3 NIT codegen file-list scope; F4 NIT mock-row note clarified).
> See `contracts/S038-F-038/feedback.md` for the full review.
...
| `packages/api-types/` | **Regen** | `make codegen` — regenerate and commit ALL output (`openapi.json` AND any generated `.ts` files) in the same commit per hard invariant #6 (F3 NIT) |
...
| `test_list_recipes_owner_id_in_query` | **F2 LOW** SQL-structural | Capture `session.execute.call_args_list[0].args[0]` (the `Select` from Query 1); compile with `literal_binds=True`; assert both `"owner_id"` and the mock user's id literal appear in the rendered SQL string. Mirrors `test_list_collections_owner_filter` in `test_sources_collections_list.py`. |
...
- All mocked recipe rows must populate all 7 ORM-mapped attributes (including
  `definition`) for completeness / future-proofing — Pydantic's
  `from_attributes=True` only reads the 5 fields declared on `RecipeListItem`,
  so the extras are not strictly required for `model_validate`, but populating
  them avoids `MagicMock` attribute-access surprises if the code path changes
  (F4 NIT clarification).
...
## 8. Open Questions — RESOLVED by reviewer

**OQ-1: Should MVP support `limit`/`offset` pagination?** → **Option A confirmed
(no pagination for MVP).** Reviewer F1 MEDIUM accepted Option A as appropriate
for MVP given the spec text "returns all recipes" and the small expected
cardinality. `total` is still returned in the envelope for forward-compat.

**OQ-2: Should `definition` be included in list items?** → **No.** `definition`
remains excluded; the slim `RecipeListItem` schema is correct. Full record
retrieval belongs on the detail endpoint (future F-039).

**OQ-3: Ordering — `created_at DESC` vs. `id ASC`?** → **`created_at DESC,
id DESC` accepted** (proposal's choice). Reviewer noted that `list_collections`
uses `id ASC` for stable pagination, but since F-038 is unpaginated the
newest-first browse semantics win.

(Original analysis preserved below for traceability.)

**OQ-1 [resolved Option A]: Should MVP support `limit`/`offset` pagination?**
