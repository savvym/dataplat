...
# F-029: Launch an asset backfill for attr_lang. Structurally identical to
# the attr_quality backfill mutation; only the asset path differs.
...
# F-030: Launch an asset backfill for attr_minhash. Structurally identical to
# the attr_lang backfill mutation; only the asset path and title differ.
_LAUNCH_ATTR_MINHASH_BACKFILL_MUTATION = """
mutation LaunchAttrMinhashBackfill($backfillParams: LaunchBackfillParams!) {
  launchPartitionBackfill(backfillParams: $backfillParams) {
    __typename
    ... on LaunchBackfillSuccess { backfillId }
    ... on PartitionSetNotFoundError { message }
...
class DagsterGateway:
    ...
    async def launch_attr_lang_backfill(self, partition_keys: list[str]) -> str:
        """Launch an asset backfill for attr_lang over the given partition keys (F-029).

        Executes the `launchPartitionBackfill` GraphQL mutation. The backfill
        enqueues one per-partition Dagster run per key in `partition_keys`.
        The attr_lang asset performs a column-mode update on existing
        producer_asset='chunks' rows — no new rows created.
    ...
    async def launch_attr_minhash_backfill(self, partition_keys: list[str]) -> str:
        """Launch an asset backfill for attr_minhash over the given partition keys (F-030).

        Executes the `launchPartitionBackfill` GraphQL mutation. The backfill
        enqueues one per-partition Dagster run per key in `partition_keys`.
        The attr_minhash asset performs a column-mode update on existing
        producer_asset='chunks' rows — no new rows created.
        ...
        """
        payload = {
            "query": _LAUNCH_ATTR_MINHASH_BACKFILL_MUTATION,
            "variables": {
                "backfillParams": {
                    "assetSelection": [{"path": ["attr_minhash"]}],
                    "partitionNames": partition_keys,
                    "title": "F-030 attr_minhash",
                }
            },
        }
        try:
